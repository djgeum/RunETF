...
def main():
    ...
    # 4) 텔레그램 전송 (통과분만, 0건이면 '없음' 메시지) → 실제 전송 성공한 행 리스트 반환
    sent_rows = telegram_send.send_report(kept, today_str=now[:10])
    ...
    # 5) notified 기록 (실제 전송 성공한 것만 → 실패분은 다음 실행에 자동 재시도)
    if sent_rows:
        select_new.append_notified(sent_rows, now_str=now)
        print(f"[기록] notified {len(sent_rows)}건 추가")
