...
MODEL = "gemini-flash-latest"              # 기본값. 막히면 _resolve_model 이 사용가능 모델 자동 선택.
_RESOLVED_MODEL = None                      # 자동 선택 결과 캐시
...
def _resolve_model(client):
    """MODEL 이 사용 가능하면 그대로, 아니면 사용가능한 flash 계열을 자동 선택."""
    ...
    if names:
        if MODEL in names:
            ...
            chosen = flash[0] if flash else names[0]
        print(f"[모델] 사용 모델: {chosen}")
...
# ===========================================================================
# 제목만 배치 스크리닝 (본문/URL 안 읽음 → 저렴·빠름, 429 방지)
#   - my_criteria 기준으로 '제목만' 보고 적합한 것만 남김
#   - 여러 공고를 한 번의 호출로 묶어 판정 (BATCH_SIZE 단위)
# ===========================================================================
BATCH_SIZE = 80
...
def _parse_int_list(text):
    if not text:
        return []
    m = re.search(r"\[[^\]]*\]", text, re.S)
    src = m.group(0) if m else text
    return [int(x) for x in re.findall(r"\d+", src)]


def _screen_chunk(client, chunk, criteria, model):
    listing = "\n".join(
        f"{i + 1}. {r.get(COL_COMPANY, '')} | {r.get(COL_TITLE, '')}"
        for i, r in enumerate(chunk)
    )
    prompt = (
        criteria
        + "\n\n======================\n[제목 스크리닝 지시]\n======================\n"
          "아래 채용공고 '제목'만 보고, 내가 지원하기에 적합한 것의 '번호'만 골라줘.\n"
          "- 명백히 안 맞는 직무(재무/회계/개발/생산/품질/인사/총무/연구/보안 등)는 제외.\n"
          "- 제목만으론 애매하면 '포함'해줘(놓치지 않게).\n"
          "- 다른 말 없이, 적합한 번호만 JSON 배열로만 답해. 예: [1,4,5]\n\n"
        + listing
    )
    try:
        r = client.models.generate_content(model=model, contents=prompt)
        nums = _parse_int_list(r.text or "")
        return [chunk[n - 1] for n in nums if 1 <= n <= len(chunk)]
    except Exception as e:
        print(f"  [제목스크리닝] 청크 오류 → 전체 유지 ({e})")
        return list(chunk)              # 실패 시 보수적으로 유지(놓침 방지)


def batch_title_screen(rows, criteria=None):
    """제목만으로 배치 스크리닝. 적합한 공고 리스트 반환(본문/URL 안 읽음)."""
    if not rows:
        return []
    if criteria is None:
        criteria = load_criteria()
    ...
    for i in range(0, len(rows), BATCH_SIZE):
        ...
        kept.extend(_screen_chunk(client, chunk, criteria, model))
        time.sleep(CALL_DELAY_SEC)
    print(f"[제목 스크리닝] {len(rows)}건 → {len(kept)}건 통과")
    return kept
...
if __name__ == "__main__":
    ...
    # 사용 가능한 모델 목록 보기:  python ai_filter.py --models
    if "--models" in sys.argv:
        for n in list_models(get_client()):
            print(" ", n)
        raise SystemExit(0)
