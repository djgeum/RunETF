...
"""
saramin_bot_main.py
사람인 수집결과에서 '타겟 기업' 공고만 골라내는 스크립트
(jobkorea_bot_main.py 와 매칭 로직 동일, 입력/출력 파일명만 사람인용)

흐름
  1) saramin_list.csv        : 사람인 봇이 수집한 전체 공고 (입력)
  2) target_companies.xlsx   : 타겟 기업 목록 (Sheet1 A열 '기업명'만 사용)
  3) 두 파일의 기업명을 정규화 매칭 -> 타겟에 해당하는 공고만
  4) saramin_main.csv        : 매칭된 공고만 저장 (출력)
...
# 설정
# ===========================================================================
INPUT_CSV = "saramin_list.csv"           # 사람인 전체 수집 결과 (입력)
TARGET_XLSX = "target_companies.xlsx"    # 타겟 기업 목록
OUTPUT_CSV = "saramin_main.csv"          # 매칭 결과 (출력)  ※ 실행 시 덮어써짐

TARGET_SHEET = "Sheet1"                   # 타겟 시트명 (없으면 첫 시트 사용)
TARGET_COMPANY_HEADER = "기업명"          # 타겟에서 사용할 A열 헤더
CSV_COMPANY_COL = "기업명"                # 사람인 CSV의 회사명 컬럼
