...
방식
  - 8개 키워드를 순서대로 URL 검색: /jobs?q=키워드&field=all&career_level=1&page=N
    (피플앤잡은 q 만으로는 검색이 안 먹고 field 를 함께 줘야 작동)
  - 각 카드(div.jd-card): 제목·회사명·근무지역·경력레벨·등록일·마감일·링크(job_id)
  - 등록일 최신순 정렬 → 최근 RECENT_DAYS 를 넘어서면 조기 중단
  - job_id 중복 제거(첫 키워드/첫 데이터 우선)
  - ★ 제목에 EXCLUDE_TITLE_WORDS 가 들어간 공고는 아예 저장하지 않음
  - CSV 컬럼은 잡코리아 list.csv 와 동일한 형식
  - 재실행 시 기존 파일에 신규만 누적 + 수집 60일 지난 공고는 자동 삭제
...
def main():
    ...
    print(f"제외 단어: {EXCLUDE_TITLE_WORDS} / 검색범위: {SEARCH_FIELD} / 직급: {CAREER_LEVEL or